...
- 🖥️ **Modern UI** — React + TypeScript interface powered by a FastAPI backend
...
4. **Multi-Video Comparison** — Interactive HTML table comparing videos across depth, difficulty, teaching style, and audience fit — with clickable YouTube thumbnails for each video
5. **Assignment Generation** — Produce educational assignments derived from the analyzed content
...
| Column | Details |
|---|---|
| **🎬 Video** | YouTube thumbnail (96×54px) — click to open the video in a new tab |
...
| **⚠ No data** | Shown when AI analysis hasn't run yet (summaries required first) |

> **Note:** Difficulty, Teaching Style, and Content Depth are generated by the AI insight engine and require successful AI summarization to populate. If the API is rate-limited or summaries fail, these columns show an amber **⚠ No data** badge.
...
> The Vite dev server proxies all `/api` requests to the FastAPI backend via `frontend/vite.config.ts`.
